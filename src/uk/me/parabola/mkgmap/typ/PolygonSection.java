...
class PolygonSection implements ProcessSection {
	...
	private int decodeFontStyle(String value) {
		if (value.startsWith("NoLabel")) {
			return 1;
		} else {
			System.out.printf("FontStyle value %s unknown", value);
			return 0;
		}
	}
	 ...
	 * different cases.
	 */
	private void readXpm(TokenScanner scanner, String data) {
		String first = data.substring(1, data.length() - 1);
	 ...
	 */
	private ColourInfo readColourInfo(TokenScanner scanner, int numberColours, int charsPerPixel) {
	 ...
	 * always called.
	 */
	private BitmapImage readImage(TokenScanner scanner, int w, int h, int cpp, ColourInfo colourInfo) {
