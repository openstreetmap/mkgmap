...
public class PointDisplayer {
	...
	public PointDisplayer(ReadStrategy reader, PrintStream outStream) {
		this.reader = reader;
		this.out = outStream;
	}
	...
	private void printTypes(int start, int itemsize, int size, int stylestart) {
		...
		for (long pos = start; pos < end; pos += itemsize) {
			...
			item.setBytes(b);
			item.addText("unknown %x", (((b[1]&0xff) << 8) + (b[0]&0xff))>>1);
